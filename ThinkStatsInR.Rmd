...
adultWeight <- read.fwf('~/Documents/CodeWork/ThinkStats2/code/CDBRFS08.ASC.gz', 
                        widths = c(1253, 5), col.names = c('v1', 'wtkg2'))
```

```{r}
#clean weight, as per the codebook
...
mu <- adultWeight$wtkg2 %>%
  mean(na.rm=TRUE)
sigma <- adultWeight$wtkg2 %>%
  sd(na.rm = TRUE)
...
logmu <- adultWeight$wtkg2 %>%
  log10 %>%
  mean(na.rm=TRUE)
logsigma <- adultWeight$wtkg2 %>%
  log10 %>%
  sd(na.rm=TRUE)
...
adultWeight %>%
  ...
  theme(legend.position = c(0.8, 0.2)) 
```
The two figures above show the distribution of adult weights on a linear scale with a normal model, and on a log scale with a lognormal model, respectively. The lognormal model is a better fit, but this representation of the data does not make the dierence particularly dramatic.
...
```{r}
ggplot()+
  stat_qq(aes(sample=adultWeight$wtkg2, color='weights'), 
          na.rm = TRUE, geom="line", distribution = qnorm) +
  #the model as per mu and sigma
  geom_qq(aes(sample = rnorm(length(adultWeight$wtkg2), mean=mu, sd=sigma), 
              color='model'), geom='line') +
...
ggplot()+
  ...
  stat_qq(aes(sample=log10(adultWeight$wtkg2), color='weights'), 
          na.rm = TRUE, geom="line", distribution = qnorm) +
  #the model as per mu and sigma
  geom_qq(aes(sample = rnorm(length(adultWeight$wtkg2), mean=logmu, sd=logsigma), 
              color='model'), geom='line') +
  scale_color_manual('Cases', values = c('blue', 'red')) +
